...
interface Step2VesselLocationsImportProps {
  formData: {
    vesselId?: number
    eta?: string
  ...
  onUpdate: (data: Partial<Step2VesselLocationsImportProps['formData']>) => void
  errors?: Record<string, string>
}

export function Step2VesselLocationsImport({
  formData,
  onUpdate,
  errors,
}: Step2VesselLocationsImportProps) {
  ...
  // Load vessel details and auto-populate wharf to "from" field for import
  useEffect(() => {
    if (formData.vesselId) {
        ...
        .then(async (data) => {
          if (data.success && data.vessel) {
            ...
            if (vessel.eta && !formData.eta) {
              updates.eta = vessel.eta
            }
            if (vessel.availability !== undefined && formData.availability === undefined) {
              updates.availability = !!vessel.availability
            }
            if (vessel.storageStart && !formData.storageStart) {
              updates.storageStart = vessel.storageStart
            }
            if (vessel.firstFreeImportDate && !formData.firstFreeImportDate) {
              updates.firstFreeImportDate = vessel.firstFreeImportDate
            }

            // Auto-populate wharf to "from" field for import jobs (always override when vessel is selected)
            if (vessel.wharfId) {
              const wharfId =
                typeof vessel.wharfId === 'object' ? vessel.wharfId.id : vessel.wharfId
              const wharfValue = `wharves:${wharfId}`

              console.log('[Step2 Import] Auto-filling wharf to from field:', {
                wharfId,
                wharfValue,
              })
              ...
              try {
                ...
                if (wharfRes.ok) {
                  ...
                  console.log('[Step2 Import] Wharf API response:', wharfData)
                  if (wharfData.success && wharfData.wharf) {
                    const wh = wharfData.wharf as Wharf
                    setSelectedWharf(wh)
                    updates.fromId = wharfValue
                    updates.fromAddress = wh.address?.street || ''
                    updates.fromCity = wh.address?.city || ''
                    updates.fromState = wh.address?.state || ''
                    updates.fromPostcode = wh.address?.postcode || ''
                    console.log('[Step2 Import] Setting from field updates:', updates)
                  }
                } else {
                  console.error('[Step2 Import] Failed to fetch wharf:', wharfRes.status)
                  // Still set the ID even if address fetch fails
                  updates.fromId = wharfValue
                  setSelectedWharf(null)
                }
              } catch (error) {
                console.error('[Step2 Import] Error fetching wharf:', error)
                // Still set the ID even if address fetch fails
                updates.fromId = wharfValue
                setSelectedWharf(null)
              }
      ...
      // Check if current fromId is a wharf and clear it
      if (formData.fromId) {
        const fromIdStr =
          typeof formData.fromId === 'string' ? formData.fromId : `customers:${formData.fromId}`
        if (fromIdStr.startsWith('wharves:')) {
          onUpdate({
            fromId: undefined,
            ...
            {selectedVessel && (
              <div className="space-y-3">
                <FormInput
                  ...
                  onChange={(e) =>
                    onUpdate({
                      eta: e.target.value ? new Date(e.target.value).toISOString() : undefined,
                    })
                  }
                  error={errors?.eta}
                />
