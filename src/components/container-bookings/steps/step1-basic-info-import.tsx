...
interface Step1BasicInfoImportProps {
  formData: {
  ...
  onUpdate: (data: Partial<Step1BasicInfoImportProps['formData']>) => void
  errors?: Record<string, string>
}

export function Step1BasicInfoImport({
  formData,
  onUpdate,
  errors,
}: Step1BasicInfoImportProps) {
  ...
  const loadOptions = useCallback(async () => {
    ...
    try {
      ...
      if (customersRes.ok) {
        ...
        setConsignees(customersData) // Consignees are only from customers collection
      }
  ...
  const handleCustomerCreated = (
    ...
    if (collection === 'customers') {
      ...
      setConsignees((prev) => [...prev, customer as Customer])
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <FormCombobox
        label="Consignee"
        required
        placeholder="Select consignee (customer)..."
        options={consignees.map((cust) => ({
          value: cust.id,
          label: cust.customer_name,
        }))}
        value={formData.consigneeId}
        onValueChange={(value) =>
          onUpdate({ consigneeId: typeof value === 'number' ? value : undefined })
        }
