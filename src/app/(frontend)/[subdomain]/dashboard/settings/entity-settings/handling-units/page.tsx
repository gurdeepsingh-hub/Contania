...
export default function HandlingUnitsPage() {
  ...
  const loadHandlingUnits = async () => {
    try {
      ...
      const res = await fetch(`/api/handling-units?${params.toString()}`)
      if (res.ok) {
        const data = await res.json()
  ...
  const handleDeleteUnit = async (unit: HandlingUnit) => {
    if (!confirm(`Are you sure you want to delete ${unit.name}? This action cannot be undone.`)) {
      return
    }

    try {
      const res = await fetch(`/api/handling-units/${unit.id}`, {
        method: 'DELETE',
      })
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">Configure handling unit types</p>
        </div>
        <Button
      ...
      <Dialog open={showAddForm} onOpenChange={(open) => !open && handleCancel()}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingUnit ? 'Edit Handling Unit' : 'Add New Handling Unit'}
            </DialogTitle>
            <DialogDescription>
      ...
      <Card>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                Handling Units ({totalDocs})
              </CardTitle>
              <CardDescription>Manage handling unit types</CardDescription>
            </div>
            <div className="relative max-w-sm w-full sm:w-auto">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input
                placeholder="Search handling units..."
                value={searchQuery}
        ...
        <CardContent>
          {loadingUnits ? (
            <div className="text-center py-8 text-muted-foreground">Loading handling units...</div>
          ) : handlingUnits.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              {searchQuery
                ? 'No handling units found matching your search'
            ...
            <>
              ...
              {totalPages > 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 pt-4 border-t">
                  <div className="text-sm text-muted-foreground">
                    Showing {handlingUnits.length > 0 ? (page - 1) * limit + 1 : 0} to{' '}
                    {Math.min(page * limit, totalDocs)} of {totalDocs} handling units
                  </div>
