...
import { Building2, ArrowLeft, Mail, Phone, Globe, MapPin, Edit } from 'lucide-react'
import { hasViewPermission, canManageRoles, isAdmin } from '@/lib/permissions'
...
export default function CompanySettingsPage() {
  ...
  useEffect(() => {
    if (authChecked && currentUser) {
      // Only allow admins and super admins (super admins have role === 'superadmin' in users collection)
      // For tenant users, check if they can manage roles (admin) or are super admin
      const isSuperAdmin = (currentUser as { role?: string; collection?: string }).role === 'superadmin' ||
        (currentUser as { collection?: string }).collection === 'users'
      const isTenantAdmin = canManageRoles(currentUser) || isAdmin(currentUser)
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        {canEdit && (
          <Button onClick={() => router.push('/dashboard/settings/company-settings/edit')}>
