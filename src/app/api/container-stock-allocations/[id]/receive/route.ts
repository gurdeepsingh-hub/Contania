...
/**
 * POST /api/container-stock-allocations/[id]/receive
 * Receive stock for import job (transition from expected to received)
 */
export async function POST(
  ...
  try {
    ...
    if (!isImport) {
      return NextResponse.json(
        { message: 'Receive action is only for import jobs' },
        { status: 400 }
      )
    }

    // Update allocation with received quantities and stage
    ...
    return NextResponse.json({ message: 'Failed to receive stock' }, { status: 500 })
  }
}
