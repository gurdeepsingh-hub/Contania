  ...
  try {
    ...
    for (const allocationItem of allocations) {
      ...
      if (lpnIds && Array.isArray(lpnIds) && lpnIds.length > 0) {
        ...
        ;(allocationItem as any).partialAllocations = partialAllocations
      } else {
        errors.push({
          productLineIndex,
      ...
      let primaryLocation = ''
      const partialAllocations = (allocationItem as any).partialAllocations || []
        ...
        if (qty < 0) {
          ...
          const loosenedStockRecord = await payload.create({
            ...
            data: {
              ...
              loosenedBatchNumber: batchNumber,
              parentLpnId: lpn.id,
              allocationStatus: 'allocated', // Immediately allocate to allocation
              containerStockAllocationId: allocationId,
              containerDetailId: containerDetailId,
              allocatedAt: new Date().toISOString(),
              allocatedBy: currentUser?.id,
